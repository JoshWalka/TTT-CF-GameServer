CS352 Team Project
Group 2 (Frank Nicolaro, Joshua Walker, Kyle Rusinko)
Readme-pGames.txt file

Note: Utilized the helper programs provided in the sample program from class,
as well as the helper methods for the reading and writing of files (readRecordAt(),
writeRecordAt()).

How to compile:
   For the client:
   gcc -o client pGame-client-main-group2.c client-thread-2021.c

   For the server:
   gcc -pthread -o gameServer pGame-server-main-group2.c server-thread-2021.c

How to run:
   For the server:
   ./gameServer 17200 scoreboard.bin

   For the client:
   ./client freebsd1.cs.scranton.edu 17200 freebsd'x'.cs.scranton.edu

   (Insert the number server which you are running on into 'x' without
   quotes such as 2 or 3)
...
How it works:
   ...
   The file system is new to "pGame" ("pick game", "persisting game", whatever
   you prefer) and, on starting the server, the scoreboard reads in a record
   of each player one at a time from 0 <= i < 10. After reading in the file and
   PlayerRecords, the main() of server will start a thread called updateFile(), which
   waits for 5 minutes for the scoreboard to update. When the 5 minutes have passed,
   the PlayerRecords will be written back into the file. This continues until the 
   server terminates. 
